    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            if batch_dict.get('batch_index', None) is not None:
                ...
                batch_mask = index
            #box_preds:eval_shape:torch.Size([321408, 7])
            #>批次大小为1，当前批次就一个样本的检测结果，取出该样本检测框空间信息检测结果
            box_preds = batch_dict['batch_box_preds'][batch_mask]
            src_box_preds = box_preds
            
            if not isinstance(batch_dict['batch_cls_preds'], list):
                #cls_preds:eval_shape:torch.Size([321408, 3])
                #>批次大小为1，当前批次就一个样本的检测结果，取出该样本检测框分类结果
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                #cls_preds:eval_shape:torch.Size([321408])
                #>321408个检测框在类别维度上取3个类别中的最大值得到cls_preds
                #label_preds:eval_shape:torch.Size([321408])
                #>记录这321408个检测框所保留类别的索引值
                ...
                #>通过筛选候选框得到最终保留的候选框索引
                #selected_scores:eval_shape:torch.Size([9])
                #>通过筛选候选框得到最终保留的候选框分类信息
                ...
                #final_scores:eval_shape:torch.Size([9])
                #>获取最终保留的候选框的分类信息
                final_scores = selected_scores
                #final_labels:eval_shape:torch.Size([9])
                #>获取最终保留的候选框的类别标签
                final_labels = label_preds[selected]
                #final_boxes:eval_shape:torch.Size([9, 7])
                #>获取最终保留的候选框的空间信息
